<?php
/**
 * Envato CHild Github class.
 *
 * @package Envato_Child
 */

if ( ! class_exists( 'Envato_Child_Github' ) ) :

	/**
	 * Creates the connection between Github to install & update the Envato Child plugin.
	 *
	 * @class Envato_Child_Github
	 * @version 1.0.0
	 * @since 1.0.0
	 */
	class Envato_Child_Github {
		 ...
		 * @type string
		 */
		const AJAX_ACTION = 'envato_child_dismiss_notice';
		 ...
		 *
		 * @see Envato_Child_Github()
		 * @uses Envato_Child_Github::init_actions() Setup hooks and actions.
		 *
		 * @since 1.0.0
		 * @static
		 * @return object The one true Envato_Child_Github.
		 * @codeCoverageIgnore
		 */
		 ...
		 *
		 * @see Envato_Child_Github::instance()
		 *
		 * @since 1.0.0
		 * @access private
		 * @codeCoverageIgnore
		...
		public function __clone() {
			_doing_it_wrong( __FUNCTION__, esc_html__( 'Cheatin&#8217; huh?', 'envato-child' ), '1.0.0' );
		}
		...
		/**
		 * Disables requests to the wp.org repository for Envato Child.
		 *
		 * @since 1.0.0
		 *
		...
		public function update_check( $request, $url ) {
			...
			if ( false !== strpos( $url, '//api.wordpress.org/plugins/update-check/1.1/' ) ) {
				...
				// Remove the Envato Child.
				unset( $data->plugins->{'envato-child/envato-child.php'} );
		...
		public function plugins_api( $api, $action, $args ) {
			if ( isset( $args->slug ) && 'envato-child' === $args->slug ) {
		...
		public function update_plugins( $transient ) {
			...
			if ( 'activated' === $state ) {
				$api_check = $this->api_check();
				if ( is_object( $api_check ) && version_compare( envato_child()->get_version(), $api_check->version, '<' ) ) {
					$transient->response['envato-child/envato-child.php'] = (object) array(
						'slug'        => 'envato-child',
		...
		public function state() {
			$option = 'envato_child_state';
			$active_plugins = apply_filters( 'active_plugins', get_option( 'active_plugins' ) );
			if ( in_array( 'envato-child/envato-child.php', $active_plugins ) ) {
				$state = 'activated';
				...
				foreach ( array_keys( get_plugins() ) as $plugin ) {
					if ( strpos( $plugin, 'envato-child.php' ) !== false ) {
						$state = 'deactivated';
						update_option( $option, $state );
					}
				}
		...
		public function notice() {
			...
			$slug = 'envato-child';
			$state = get_option( 'envato_child_state' );
			...
			if ( 'deactivated' === $state ) {
				...
				$message = sprintf(
					esc_html__( '%sActivate the Envato Child plugin%s to get updates for your ThemeForest and CodeCanyon items.', 'envato-child' ),
				...
				$message = sprintf(
					esc_html__( '%sInstall the Envato Child plugin%s to get updates for your ThemeForest and CodeCanyon items.', 'envato-child' ),
			...
			if ( isset( $message ) ) {
				?>
				<div class="updated envato-child-notice notice is-dismissible">
					<p><?php echo wp_kses_post( $message ); ?></p>
				</div>
				<script>
				jQuery( document ).ready( function( $ ) {
					$( document ).on( 'click', '.envato-child-notice .notice-dismiss', function() {
						$.ajax( {
							url: ajaxurl,
							data: {
	...
	if ( ! function_exists( 'envato_child_github' ) ) :
		/**
		 * Envato_Child_Github Instance
		 *
		 * @since 1.0.0
		 *
		 * @return Envato_Child_Github
		 */
		function envato_child_github() {
			return Envato_Child_Github::instance();
		}
	endif;

	/**
	 * Loads the main instance of Envato_Child_Github
	 *
	 * @since 1.0.0
	 */
	add_action( 'after_setup_theme', 'envato_child_github', 99 );
